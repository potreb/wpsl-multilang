...
<style>
	<?php foreach ( $plugin->get_blog_sites_id() as $blog_id => $url ) : ?>
		...
		#wpadminbar .quicklinks #wp-admin-bar-blog-<?php echo $blog_id; ?> .blavatar:before {
			content: url('<?php echo  esc_url( $plugin->get_assets( 'images/flags' ) ) . $plugin->get_short_lang( $blog_id ) . '.png'; ?>') !important;
			top: 0;
		...
		.wp-admin #wpadminbar #wp-admin-bar-site-name > .ab-item::before {
			content: url('<?php echo  esc_url( $plugin->get_assets( 'images/flags' ) ) . $plugin->get_short_lang( $plugin->get_current_blog_id() ) . '.png'; ?>');
			top: 0;
		}
	<?php endforeach; ?>
</style>
